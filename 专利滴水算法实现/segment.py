...
⚠️ 2026-09-18 依据强度核查（**不要据此改默认值，先读 ADR**）：
    上述依据来自表3-1，而表3-1 是论文里**样本最小**的一张表（两个具名字）。
    论文 §3.4.5（p58）的**集级**统计说 B 是最差的指标——
      集A A=99.06% **B=96.26%** C=97.20% **D=99.06%**；集B A=91.96% **B=93.75%** C=94.64% **D=95.53%**，
      原文「优化指标B 在四个优化指标中表现最差」，论文最终选 **D**。
    实测（表3-3 十个真实不粘连字，l2 轨迹穿墨量）：B=38、D=15 → D 不差于 B。
    暂不改默认值的原因与翻案条件见 `docs/adr/0003-keep-metric2-B.md`。
"""
