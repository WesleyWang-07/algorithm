...
## ⚠️ 与 Python 版的差异：**零代码差异，一处待复核的文字滞后**（2026-09-17 核对）
...
**结论：两套实现当前不存在算法分歧。** 上一版本文档标题写的"两处已知差异"是**措辞错误**——
它把"代码有分歧"和"文档没跟上"混为一谈，会让人误以为 MATLAB 版不可信。**实际是后者。**

**待复核的一处文字滞后（非差异，是未验证项）**：Python 版在 2026-09-17 把真实谱字A
的锚点行号从 `96` 更正为 `(92,46)`（此前 `96` 是早期误读，见
`../专利滴水算法实现/验证报告.md` 4.1 节口径提醒）。**MATLAB 侧从未复核过这个数字。**

> ⚠️ 这不代表 MATLAB 版算错了 —— `run_real.m:25` 是从 `res.intersect` **动态取值**打印的，
> 没有写死任何数字。若重跑 `matlab -batch "run_real"` 输出 92，说明两版一致；
> **若输出 96 或别的值，则说明终止逻辑存在实质差异，需要单独排查。**
> 在此之前，**这个疑点是"未验证"，不是"已知差异"** —— 两者性质不同，别混淆。
